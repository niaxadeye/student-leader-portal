...
import { ChallengeDrawCard } from './challenge-draw-card'
import { ChallengeBriefingCard } from './challenge-briefing-card'
...
export function ChallengeFormPage() {
  ...
  const showForm = Boolean(
    challenge?.accepts_submissions && challenge.status === 'PUBLISHED' && (challenge.fields?.length ?? 0) > 0,
  )
  const { data: sub, isLoading: subLoading } = useSubmission(showForm ? challengeId : undefined)
  const form = useSubmissionForm(showForm ? challenge : undefined, showForm ? sub : undefined)
  const submitMut = useSubmit(challengeId ?? '')

  if (isLoading || !challenge || (showForm && (subLoading || !sub))) {
    return (
  ...
  const readOnly = Boolean(sub?.locked)
  const revision = sub?.current_revision_number ?? 0
  ...
  return (
    <div className="flex flex-col gap-6 pb-24">
      <ChallengeFormHeader
        ...
        saveState={showForm ? form.saveState : 'idle'}
        revision={revision}
        revisionAt={sub?.last_resubmitted_at ?? sub?.submitted_at}
        showStatus={showForm}
      />

      <ChallengeBriefingCard briefing={challenge.briefing} />
      <ChallengeDrawCard challengeId={challenge.id} />
