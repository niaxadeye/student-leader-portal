...
func (r *Repo) List(ctx context.Context, f ListFilter) ([]User, int, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT u.id, u.login, u.full_name, u.email, u.organization, u.status,
		...
		LIMIT $4 OFFSET $5`, f.Search, f.Status, f.Role, f.Limit, f.Offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var u User
		if err := rows.Scan(&u.ID, &u.Login, &u.FullName, &u.Email, &u.Organization,
...
func (r *Repo) attachRoles(ctx context.Context, users []User, ids []string) error {
	...
	rows, err := r.pool.Query(ctx, `
		SELECT ur.user_id, rl.code, ur.scope_type, ur.scope_id
	...
	for rows.Next() {
		...
		if err := rows.Scan(&uid, &ra.Code, &ra.ScopeType, &ra.ScopeID); err != nil {
			return err
		}
		if i, ok := idx[uid]; ok {
...
func (r *Repo) ByID(ctx context.Context, id string) (*User, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT id, login, full_name, email, organization, status,
		       must_change_password, last_login_at, created_at
		FROM users WHERE id=$1 AND deleted_at IS NULL`, id).
		Scan(&u.ID, &u.Login, &u.FullName, &u.Email, &u.Organization, &u.Status,
...
func (r *Repo) rolesOf(ctx context.Context, userID string) ([]RoleAssignment, error) {
	...
	for rows.Next() {
		var ra RoleAssignment
		if err := rows.Scan(&ra.Code, &ra.ScopeType, &ra.ScopeID); err != nil {
			return nil, err
		}
		out = append(out, ra)
	}
