...
var validRoles = map[string]bool{"MEGA_ADMIN": true, "SUPER_ADMIN": true, "ADMIN": true, "STAFF": true, "CONTESTANT": true}
...
func createUser(ctx context.Context, pool *pgxpool.Pool, args []string) error {
	...
	if !validRoles[role] {
		return fmt.Errorf("role must be one of MEGA_ADMIN|SUPER_ADMIN|ADMIN|STAFF|CONTESTANT")
	}
	if len(password) < 10 {
